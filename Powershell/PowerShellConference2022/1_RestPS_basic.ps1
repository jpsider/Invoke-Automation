...
```powershell
# RESTServerConsole
Find-Module -Name RestPS -RequiredVersion 7.0.8 -Repository PSGallery
...
```powershell
# RESTServerConsole
Get-Module -Name RestPS -ListAvailable
...
$RestMethodParamsHttp = @{
  Uri = 'http://localhost:8080/endpoint/status'
  Method = 'Get'
  UseBasicParsing = $true
}
Invoke-RestMethod @RestMethodParamsHttp
1 # <---- Actual return is a single digit.
# Shutdown the Endpoint.
$RestMethodParamsHttp = @{
  Uri = 'http://localhost:8080/endpoint/shutdown'
  Method = 'Get'
